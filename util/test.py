...
def writeBug(bug_id, interface_name, request, response, res_check):
    ...
    step = '[请求报文]<br />'+request+'<br/>'+'[预期结果]<br/>' + \
        res_check+'<br/>'+'<br/>'+'[响应报文]<br />'+'<br/>'+response
    ...
    coon = MySQLdb.connect(user='root', passwd='123456',
                           db='bugfree', port=3306, host='127.0.0.1', charset='utf8')
    # 建立游标
    cursor = coon.cursor()
    # 执行sql
    cursor.execute(sql)
