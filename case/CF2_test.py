...
class CF2(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        l2 = getConfig("CF2", "action")
        action = eval(l2)
        l3 = getConfig("CF2", "position")
        position = eval(l3)
        l4 = getConfig("CF2", "text")
        text = eval(l4)
        start_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        order_type = getConfig("CF2", "order_type")
        globals()["xp"] = None
        globals()["oid"] = None
    ...
    def test_11order_submit(self):
        ...
        n3 = dr.find_elements_by_css_selector('[class="btn btn-danger btn-sm"]')[2]
        n3.click()
        ...
        m11 = dr.find_element_by_id('applicationAmount')
        m11.send_keys(1100)
        m12 = dr.find_element_by_id('doorAmount')
        m12.send_keys(0)
        m13 = dr.find_element_by_id('remark')
        m13.send_keys('测试集中式收房进件')
