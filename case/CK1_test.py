...
class CK1(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        burl = getConfig("CK1", "burl")
        eurl = getConfig("Eh5", "url")
        captcha = getConfig("CK1", "captcha")
        l1 = getConfig("CK1", "login_list")
        login_list = eval(l1)
        name = list(login_list.keys())
        l2 = getConfig("CK1", "action")
        action = eval(l2)
        l3 = getConfig("CK1", "position")
        position = eval(l3)
        l4 = getConfig("CK1", "text")
        text = eval(l4)
        start_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        order_type = getConfig("CK1", "order_type")
        globals()["xp"] = None
        globals()["oid"] = None
    ...
    def test_11order_submit(self):
        ...
        n3 = dr.find_elements_by_css_selector('[class="btn btn-danger btn-sm"]')[3]
        n3.click()
        ...
        n8 = dr.find_element_by_xpath('//*[@id="1_' + order_id + '"]/td[6]/button')
        n8.click()
        ...
        m1 = dr.find_element_by_name('housingInfoBean.landlordName')
        m1.clear()
        m2 = dr.find_element_by_name('housingInfoBean.landlordIdNo')
        m2.clear()
        m3 = dr.find_element_by_name('housingInfoBean.landlordCardNo')
        m3.clear()
        m4 = dr.find_element_by_name('housingInfoBean.landlordAccountingBranch')
        m4.clear()
        m1.send_keys(fake.name())
        m2.send_keys(get_id_no())
        m3.send_keys(get_card_no()[1])
        m4.send_keys(fake.street_name() + '支行')
        ...
        l3.clear()
        m5 = dr.find_element_by_name('housingInfoBean.ownerPhone')
        m5.clear()
        m6 = dr.find_element_by_name('housingInfoBean.owner')
        m6.clear()
        m7 = dr.find_element_by_name('housingInfoBean.cardId')
        m7.clear()
        m8 = dr.find_element_by_name('housingInfoBean.propertyAddress')
        m8.clear()
        m9 = dr.find_element_by_name('houseAddress')
        m9.clear()

        l1.send_keys(fake.company())
        l2.send_keys(fake.address())
        l3.send_keys(get_corp_no())
        m5.send_keys(get_phone_no())
        m6.send_keys(fake.name())
        m7.send_keys(get_id_no())
        m8.send_keys(fake.address())
        m9.send_keys(fake.address())

        s2 = Select(dr.find_element_by_id('borrowType'))
        s2.select_by_value('07')
        s3 = Select(dr.find_element_by_id('timeLimit'))
        s3.select_by_index(1)

        m10 = dr.find_element_by_id('applicationAmount')
        m10.send_keys(1100)
        m11 = dr.find_element_by_id('doorAmount')
        m11.send_keys(0)
        m12 = dr.find_element_by_id('remark')
        m12.send_keys('测试集中式装修进件')
        ...
        # 图片上传
        frame_id = 2
        list1 = ['num_0101', 'num_0102', 'num_0104', 'num_0105', 'num_0106', 'num_0107', 'num_0108', 'num_0113']
        list2 = ['房东身份证明', '产权证明', '收款银行卡', '房源现场照片', '双签收房合同', '现场签约照片', '付租相关', '房屋装修调查表']
