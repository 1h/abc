...
class K1(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        l2 = getConfig("K1", "action")
        action = eval(l2)
        l3 = getConfig("K1", "position")
        position = eval(l3)
        l4 = getConfig("K1", "text")
        text = eval(l4)
        start_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        order_type = getConfig("K1", "order_type")
        globals()['xp'] = None
        globals()["oid"] = None
    ...
    def test_11order_submit(self):
        ...
        n3 = dr.find_elements_by_css_selector('[class="btn btn-danger btn-sm"]')[1]
        n3.click()
        ...
        m7 = dr.find_element_by_name('applicationAmount')
        m7.send_keys(1350)
        m8 = dr.find_element_by_name('doorAmount')
        m8.send_keys(0)
